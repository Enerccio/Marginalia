...
public class LiteLlmTokenizerStrategy implements TokenizerStrategy {
    ...
    public long countTokens(AI aiEntity, String text) throws Exception {
        ...
        // LiteLLM returns either {"count": 123} or an array of tokens
        if (json.has("count")) {
            return json.get("count").getAsLong();
        } else if (json.has("tokens")) {
            return json.getAsJsonArray("tokens").size();
        }

        throw new IllegalStateException("Unrecognized response payload from LiteLLM /tokenize");
    }

    @Override
    public String getName() {
        return "LITELLM_NATIVE";
    }
}