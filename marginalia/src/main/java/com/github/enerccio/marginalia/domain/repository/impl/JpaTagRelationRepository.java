...
public class JpaTagRelationRepository extends JpaBaseRepository<TagRelation> implements TagRelationRepository {
    ...
    public List<Tag> findTagsForObject(Long objectId, String clazz) throws Exception {
        return getEntityManager().createQuery(
                        "SELECT tr.tag FROM TagRelation tr WHERE tr.objectId = :objectId AND tr.clazz = :clazz AND tr.deleted = false AND tr.tag.deleted = false", Tag.class)
    ...
    public List<Long> findObjectIdsForTag(Long tagId, String clazz) throws Exception {
        return getEntityManager().createQuery(
                        "SELECT tr.objectId FROM TagRelation tr WHERE tr.tag.id = :tagId AND tr.clazz = :clazz AND tr.deleted = false", Long.class)
    ...
    public <T extends BaseEntity> List<T> findObjectsForTag(Long tagId, Class<T> clazz) throws Exception {
        ...
        String jpql = "SELECT e FROM " + entityName + " e, TagRelation tr " +
                ...
                "AND tr.deleted = false AND e.deleted = false";
    ...
    public TagRelation findByTagAndObject(Long tagId, Long objectId, String clazz) throws Exception {
        List<TagRelation> results = getEntityManager().createQuery(
                        "SELECT tr FROM TagRelation tr WHERE tr.tag.id = :tagId AND tr.objectId = :objectId AND tr.clazz = :clazz AND tr.deleted = false", TagRelation.class)
                .setParameter("tagId", tagId)
                .setParameter("objectId", objectId)
                .setParameter("clazz", clazz)
                .setMaxResults(1)
